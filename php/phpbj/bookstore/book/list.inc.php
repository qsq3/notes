...
<form action="index.php?action=del" method="post" id="booklist"  onsubmit="javascript: return confirm('你确定要删除所选图书吗?')">
<table>
	<tr align="left" bgcolor="#cccccc">
		<th>选择</th><th>ID</th><th>图书名称</th> <th>出版商</th> <th>图书作者</th> <th>图书价格</th> <th>上架时间</th> <th>操作</th>
	</tr>
	<?php
		include "conn.inc.php";                              	//包含数据库连接文件，连接数据库
		include "classes/page.class.php";                               //包含分页类文件，加数据分页功能
		...
		if($result && mysql_num_rows($result) > 0 ) {
			...
			while(list($id, $bookname, $publisher, $author, $price, $pic, $ptime) = mysql_fetch_row($result)) {
				...
				echo '<tr>';
                echo '<td><input type="checkbox" value="'.$id.'" class="sid" name="sid[]" /></td>';
				echo '<td>'.$id.'</td>';
				echo '<td>《'.$bookname.'》</td>';
			...
			echo '<tr>';
            echo '<td><input type="checkbox" value="all" name="all" /><label>全选</label></td>';
            echo '<td><input type="submit" value="删除" name="submit" /></td>';
            echo '<td colspan="6">'.$page->fpage().'</td>';
            echo '</tr>';
		}else {
			echo '<tr><td colspan="8" align="center">没有图书被找到</td></tr>';
		}
...
<script type="text/javascript">
    ;(function(){
        ...
        var sid = booklist.getElementsByClassName("sid");
        booklist.all.onclick = function(){
            for(var i=0; i< sid.length; i++){
                sid[i].checked = booklist.all.checked;
            }
        }
    })()
</script>