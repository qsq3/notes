			...
			<?php
				...
				/* 如果用户请求删除一本图书action=del, 则条件成立 */
                    ...
                    if(isset($_POST['sid'])){
                        /*删除多个*/
                        if(!empty($_POST['sid'])){
                        $sql_pic = "SELECT pic FROM books WHERE id in (" . implode(',',$_POST["sid"]) . ")";
                        $sql = "DELETE FROM books WHERE id in (" . implode(',',$_POST["sid"]) . ")";
                    ...
                    if($result && mysql_affected_rows() > 0 ) {
                        ...
                        if(!empty($_POST['sid'])){
                            if($result_pic) {
                                while($res=mysql_fetch_assoc($result_pic)) {
                                    delpic($res['pic']);
                                }
                            }
                        } else {
