...
32、原生JavaScript判断是否移动设备访问 跳转手机端页面
...
101 获取圆盘布局的坐标;
102 异步加载第三方内容, 当你无法保证嵌入第三方内容比如 Youtube 视频或者一个 like/tweet 按钮可以正常工作的时候，你需要考虑用异步加载这些代码，避免阻塞整个页面加载。
103 处理money的 money.js
104 让ie支持placeholder  的 placeholder.js
105 touch事件兼容mouse的封装(包含手势判断) 原文: http://www.cnblogs.com/windyfancy/p/5173836.html
106. CSS3自适应切换高度
107 深拷贝/克隆
108 ES6 转字符编码
109 throttle/频率控制 和 debounce/空闲控制 原文: http://www.css88.com/archives/4648
110 基于 Promise 的动画库 原文: https://www.h5jun.com/post/sixty-lines-of-code-animation.html
*/
//1、原生JavaScript实现字符串长度截取
...
function addEventSamp(obj,evt,fn){ 
    ...
    return  obj.addEventListener(evt, fn, false); //w3c支持
    }else if(obj.attachEvent){ 
    return  obj.attachEvent('on'+evt,fn); //ie支持
    } 
} 
//23、原生JavaScript光标停在文字的后面，文本框获得焦点时调用
function focusLast(){ 
...
//30、原生JavaScript实现checkbox全选与全不选
function checkAll() {
    var selectall = document.getElementById("selectall");//获取checkBox的类名
...
function getScreenWidth(){
    ...
    return w ? w : Math.min(screen.width, screen.height) / (window.devicePixelRatio || 2);  
}
...
function isMobile(){
    ...
    if (typeof that._isMobile === 'boolean'){
        return that._isMobile;
    }
    var isMobileScreenSize = getScreenWidth() < 600;
    that._isMobile = isMobileScreenSize && isMobileUserAgent();
    return that._isMobile;
}
...
// 另一个检测手机端 这段代码引用自：https://github.com/binnng/device.js
var WIN = window;
...
/*
包括了邮箱、危险字符、验证长度、验证网址、验证小数、整数、
浮点数等常用的验证，有了这些代码片段，平时的表单验证也可以不需要jquery的验证插件了，
*/
...
//101 获取圆盘布局的坐标;
var  parent,children;
var  len = children.length , pW, pH ;

for(var i= 0; i<len; i++){

    pW = children[i].clientWidth/parent.clientWidth //子节点和父节点的宽度比
    pH = children[i].clientHeight/parent.clientHeight //子节点和父节点的高度比

    var radian = 2*Math.PI*(i/len); //弧度;
    children[i].style.left = ((1 + Math.sin(radian)) * 50 * ( 1 - pW )).toFixed(4)  + "%"; 
    children[i].style.top =  ((1 - Math.cos(radian)) * 50 * ( 1 - pH )).toFixed(4)  + "%"; 
}
...
//103 处理money的 money.js
function MoneyToChinase(money){
    money = Math.round(money * 100);
    if (money <= 1e+20) {
        number = new Array('零', '壹', '贰', '叁', '肆', 
                                   '伍', '陆', '柒', '捌', '玖');
        num_unit = new Array('分', '角', '元', '拾', '佰', 
                                   '仟', '万', '拾', '佰', '仟', '亿');
        str = '';
        i = 0;
        while (money > 0) {
            str = number[money % 10] + num_unit[i] + str;
...
//105 touch事件兼容mouse的封装(包含手势判断) 原文http://www.cnblogs.com/windyfancy/p/5173836.html
function TouchEvent(){
...
function slideUp(el, time) {
  if (time) {
    el.style.WebkitTransition = "height "+ time +"ms";
...
function slideDown(el, time) { 
    ...
    var startHeight = el.clientHeight && (el.clientHeight+'px')|| '0px';
...
function clone(o) {
    ...
    if ( typeof o !== 'object' || !o ){
      ...
      b.toString = o.toString;  
      b.valueOf = o.valueOf; 
    }
    return b;  
} 
// 108 ES6 转字符编码
let encodeUnicode=(strings='')=> Array.from(strings).map((s) => {
    s =s.codePointAt(0).toString(16);
    return '\\u' + new Array(5-s.length).join("0")+s
  }).join('');
...
// 109 throttle/频率控制 和 debounce/空闲控制
 /*
...
* 110 基于 Promise 的动画库
* @param duration  动画时间
* @param update   动画函数, 接受 参数 p 为运行时间比值
* @param easing   运动曲线函数;
* @return promise 实际返回的promise对象;
*/
...
var x = 200 * ep;
var blcok = document.getElementById('block');
